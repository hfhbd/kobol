...
public abstract class Rename(
    ...
    public companion object {
        ...
        public fun String.toSnakeCase(): String = toKotlinName().uppercase()
    }
        ...
        return others + tree.copy(
            ...
            types = tree.types.map {
                when (it) {
                    ...
                    is KobolIRTree.Types.Type.Class -> it.copy(name = it.name.classes())
    ...
    private fun KobolIRTree.Types.Function.Statement.updateNames(
    ): KobolIRTree.Types.Function.Statement = when (this) {
        is KobolIRTree.Types.Function.Statement.Use -> copy(
            target = target.updateNames(),
            action = action.updateNames()
        )

        is KobolIRTree.Types.Function.Statement.Static -> copy(type.copy(name = type.name.classes()))
        ...
        is KobolIRTree.Types.Function.Statement.Declaration.ObjectDeclaration -> copy(name = name.variables())
    ...
    private fun KobolIRTree.Types.Function.Statement.FunctionCall.toCamelCase() = copy(
        function = when (val function = function) {
            is KobolIRTree.Types.Function -> function.copy(name = function.name.functions())
            is KobolIRTree.Types.Type.Class -> function.copy(name = function.name.classes())
        }
    )
